...
class MLFlowDeploymntLoaderStepParameters(BaseParameters):
    """
...
def continuous_deployment_pipeline(
    data_path: str = r"C:\Users\vansh\AI and ML reading material\MLOps\OLips Customer Satisfaction Project\data\olist_customers_dataset.csv",
    min_accuracy: float = 0.92,
    workers: int = 1,
...
@pipeline(enable_cache=False, setting={"docker": docker_settings})
def inference_pipeline(pipeline_name: str, pipeline_step_name: str):
    data = dynamic_importer()
    service = prediction_service_loader(
        piepline_name = pipeline_name,
        pipeline_step_name=pipeline_step_name,
        running=False
    )
